      ...
      <div className="sort">
        {user?(<div>
          <span>{stor.userInfo.email}</span>
            <img onClick ={visibleChange} width="40" src={(stor.userInfo.avatar)?(stor.userInfo.avatar):(userPhoto)} alt={userPhoto}></img>
             {visible && 
        ...
        <Link 
        to="/login" 
        className="button button--outline button--signup">
          <span>Log in</span>
        </Link>
        )}
